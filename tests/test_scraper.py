"""Unit tests for RSS scraper behavior (network calls mocked)."""
...
def test_canonicalize_url_removes_tracking_query_params():
    scraper = RssScraper(FEED_CONFIG)
    raw_url = "https://example.com/a?utm_source=abc&id=10&fbclid=foo"
    canonical = scraper._canonicalize_url(raw_url)
    assert canonical == "https://example.com/a?id=10"
